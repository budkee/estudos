...
h = Heap(5, v); // o construtor é usado para construir uma variável temporária, e depois a operação de atribuição (cópia?) é utilizado
...
Se o objeto não tiver ponteiros para a memória alocada dinamicamente,
...
** Se você precisar de um construtor de cópias, também precisará de **
** um destruidor e um operador de atribuição (operator=) **
...
recursos. Assim, é quase certo que você precisará definir um
...
1. Não há necessidade de testar para ver se ele está sendo
inicializado por si mesmo 
2. Não há necessidade de limpar (por exemplo, excluir) um valor
existente (não há nenhum) 
...
atribuição de transferência não são fornecidos pelo compilador. Se
